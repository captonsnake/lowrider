/**
  ...
  $Revision: 42867 3ce8a1daa6cdae9957797af478309d2dc3c7fd95 $
  $Date: 2020-07-30 05:49:26 $
...
propertyDefinitions = {
  ...
  optionalStop: {title:"Optional stop", description:"Outputs optional pause after each section.", type:"boolean"},
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    // Begin Custom Tool Change code
    // Samual McCracken
    if (properties.customToolCode){
      writeBlock("o<tool_change> call [" + toolFormat.format(tool.number) + "]");
    } else {
      writeBlock("T" + toolFormat.format(tool.number), mFormat.format(6))
    }
    // End Custom Tool Change Code
    // Uncomment line below to get back the orig
    // writeBlock("T" + toolFormat.format(tool.number), mFormat.format(6));
    if (tool.comment) {
